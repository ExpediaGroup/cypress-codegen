...
export const appendCommandImports = async (filePath: string, prettierConfig: Options) => {
  ...
  const importsExist = lastNode?.leadingComments?.[0]?.value === '* Generated by cypress-codegen *';
  if (importsExist) {
    return contents;
  }
